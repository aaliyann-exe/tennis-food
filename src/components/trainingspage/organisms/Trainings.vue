...
const activeTrainings = computed(() =>
  trainingStore.trainings.filter((training) => training.status === true),
);
const archivedTrainings = computed(() =>
  trainingStore.trainings.filter((training) => training.status === false),
);
