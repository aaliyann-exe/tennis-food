  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
    ...
    <div class="h-full overflow-y-scroll bg-white shadow-xl rounded-2xl w-220">
      ...
      <div class="flex items-center justify-between py-5">
        ...
        <h2 v-if="(props.mode === 'view')" class="mt-2 ml-10 text-4xl font-semibold">{{ props.trainingData.name + ' ' + props.trainingData.lName}}</h2>

        <h2 v-if="(props.mode === 'edit')" class="mt-2 ml-10 text-4xl font-semibold">{{ props.trainingData.name + ' ' + props.trainingData.lName}}<span class="text-[20px]"> ( Edit )</span></h2>

        <button v-if="props.mode === 'view'" class="relative cursor-pointer left-65" @click="$emit('edit', props.trainingObject)">
      ...
      <form @submit.prevent="handleSave" :class="((props.mode === 'view') ? 'cursor-not-allowed ' : '') + 'p-8'">
        ...
        <div class="grid grid-cols-2">
          ...
          <TrainingInputs v-model:inputData="form.name" inputLabel="First Name" inputPlaceholder="First Name" :icon="personIcon" mustFill hasAsterisk validationMessage="First name is required" :mode="props.mode" />

          <TrainingInputs v-model:inputData="form.lName" inputLabel="Last Name" inputPlaceholder="Last Name" :icon="personIcon" :mode="props.mode" />

          <TrainingInputs v-model:inputData="form.email" type="email" inputLabel="Email" inputPlaceholder="info@youmai.com" :icon="emailIcon" mustFill  hasAsterisk :isDisabled="props.mode === 'view'" validationMessage="Email is required" :mode="props.mode" />
          
          <TrainingInputs v-model:inputData="form.phone" type="phone" inputLabel="Phone" inputPlaceholder="Enter phone number" :icon="phoneIcon" :mode="props.mode" />
          
          <TrainingInputs v-model:inputData="form.dob" inputLabel="Date of Birth" inputPlaceholder="DOB" :icon="calendarIcon" isDropDown mustFill hasAsterisk :mode="props.mode" />
