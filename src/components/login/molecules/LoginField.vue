  ...
  <div class="w-full lg:w-1/2 flex flex-col items-center justify-center px-8 md:px-24 bg-white relative container2">
    ...
    <div class="w-130">
      ...
      <form @submit.prevent="handleLogin" class="space-y-5 mt-14">
        ...
        <FormInput :label="$t('login.emailLabel')" v-model="email" :placeholder="$t('login.email')" :icon="emailIcon"
          :hasError="errorMessage.toLowerCase().includes('email')" @input-blur="emailTouched = true" />

        <FormInput :label="$t('login.password')" v-model="password" :type="showPassword ? 'text' : 'password'"
          :placeholder="$t('login.password')" :icon="passwordIcon" :showToggle="true" :isPasswordVisible="showPassword"
          :viewIcon="viewIcon" :hideIcon="hideIcon" :hasError="errorMessage.toLowerCase().includes('password')"
          @toggle-password="showPassword = !showPassword" @input-blur="passwordTouched = true" />
