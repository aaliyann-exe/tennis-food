...
const hasError = computed(() => {
  ...
  if (props.mustFill && !props.inputData) return true;
...
const errorMessages = {
  ...
  email: "Invalid email",
  phone:
    "Please enter a valid Dutch phone number (e.g. 0612345678 or +31612345678)",
  website:
...
const errorMessage = computed(() => {
  ...
  if (props.mustFill && !props.inputData) return props.validationMessage;
...
<template>
  <div
    :disabled="isReadOnly"
    :class="(isReadOnly ? 'cursor-not-allowed ' : '') + 'col-span-1 p-2'"
  >
    <label
      :class="
        (isReadOnly ? 'opacity-50 ' : '') + 'block text-sm font-semibold mb-1'
      "
    >
      {{ props.inputLabel }}
    ...
    <div v-if="isDropDown" class="relative" ref="dropdownRef">
      <div
        ...
        :class="
          (isOpen
            ? 'border-primary ring-4 ring-orange-50 bg-white '
      ...
      >
        <img
          :src="props.icon"
          class="inline-block w-5 h-5 ml-1 mr-4 cursor-not-allowed opacity-30"
        />

        <span
          :class="
            (props.inputData ? 'text-primary ' : 'text-black opacity-20 ') +
            'flex-1 font-light'
          "
        >
        ...
        <img
          :src="arrowDownIcon"
          class="relative w-3 h-3 top-2 right-2 opacity-60"
        />
      </div>
      ...
      <transition name="fade">
        <div
          v-if="isOpen"
          class="absolute z-50 w-full mt-1 overflow-hidden bg-white border border-gray-100 shadow-xl rounded-xl"
        >
          <div class="overflow-y-auto max-h-60 custom-scrollbar">
            <div
              ...
              class="flex items-center justify-between px-4 py-3 transition-colors cursor-pointer hover:bg-blue-50"
            >
              <span
                :class="[
                  'text-sm',
                  props.inputData === option
