...
const fillForm = () => {
  if (props.mode === "create") {
    ...
    form.phone =
      props.playerData.phone === "N/A" ? "" : props.playerData.phone || "";
    ...
    form.initials = props.playerData.initials;
  }
};
...
const handleSave = () => {
  ...
  if (!form.fName) return;

  if (form.email && !emailRegex.test(form.email)) return;

  if (form.phone && !phoneRegex.test(form.phone)) return;
    ...
    >
      <div class="flex items-center justify-between px-8 py-5">
        <div class="flex flex-col">
          <h2
            ...
            class="mt-2 ml-5 text-4xl font-semibold"
          >
            {{ $t("player.player") }} {{ $t("table.information") }}
          </h2>
          ...
          <h2
            ...
            class="mt-2 ml-5 text-4xl font-semibold"
          >
            {{ props.playerData.fName + " " + props.playerData.lName }}
          </h2>
        ...
        <div class="flex items-center gap-4">
          <button
            v-if="props.mode === 'view'"
            class="absolute cursor-pointer top-20 right-100"
      ...
      >
        ...
        <div class="grid grid-cols-2 gap-6">
          <PlayerInputs
            v-model:inputData="form.fName"
            :inputLabel="$t('table.fName')"
            :inputPlaceholder="$t('table.fName')"
            :icon="personIcon"
            mustFill
            hasAsterisk
            :validationMessage="
              $t('table.fName') + ' ' + $t('modalField.error')
            "
            :mode="props.mode"
          />
          ...
          <PlayerInputs
            ...
            :inputLabel="$t('table.lName')"
            :inputPlaceholder="$t('table.lName')"
            :icon="personIcon"
            :mode="props.mode"
          />
          ...
          <PlayerInputs
            ...
            :validationMessage="
              $t('table.email') + ' ' + $t('modalField.error')
            "
            :mode="props.mode"
          />
          ...
          <PlayerInputs
            ...
            type="phone"
            :inputLabel="$t('table.phone')"
            :inputPlaceholder="$t('modalField.phone')"
            :icon="phoneIcon"
            :mode="props.mode"
          />
          ...
          <PlayerInputs
            ...
            :inputLabel="$t('table.dob')"
            :inputPlaceholder="$t('modalField.dob')"
            :icon="calendarIcon"
            isDropDown
            mustFill
            hasAsterisk
          ...
          <PlayerInputs
            v-model:inputData="form.ageGroup"
            :inputLabel="$t('table.ageGroup')"
            :inputPlaceholder="
              $t('modalField.select') + ' ' + $t('table.ageGroup')
            "
            :icon="waitingIcon"
            isDropDown
            :options="ageGroups"
        ...
        <div class="grid grid-cols-2 gap-6">
          ...
          <PlayerInputs
            v-model:inputData="form.level"
            :inputLabel="$t('table.playerLevel')"
            :inputPlaceholder="
              $t('modalField.select') + ' ' + $t('table.playerLevel')
            "
            :icon="flagPersonIcon"
            isDropDown
            :options="levels"
