...
<template>
    ...
    <div :class="(isReadOnly ? 'cursor-not-allowed ' : '') + 'col-span-1 p-2'">
        ...
        <div v-if="props.isDropDown"
            :class="(isReadOnly ? 'bg-gray-50 border-gray-200 border p-2.5 cursor-not-allowed ' : 'bg-white border-gray-200 border rounded-lg p-2.5 ') + 'w-full flex items-center rounded-lg py-3'">
        ...
        <div v-else
            :class="(hasError ? 'border-red-500 ' : 'border-gray-200 focus-within:border-primary border') + (isReadOnly ? 'bg-gray-50 border-gray-200 border p-2.5 cursor-not-allowed ' : 'bg-white border rounded-lg p-2.5 ') + 'w-full flex items-center rounded-lg py-3'">

            <img :src="icon" class="w-5 h-5 mr-4 ml-3 opacity-40" />

            <input :value="props.inputData" :disabled="isReadOnly" :type="props.type === 'phone' ? 'tel' : 'text'"
                @blur="fieldTouched = true" @input="handleInput" :placeholder="props.inputPlaceholder"
                class="w-full outline-none bg-transparent text-primary placeholder:text-black placeholder:opacity-10">
