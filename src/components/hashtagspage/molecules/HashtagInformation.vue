  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
    ...
    <div class="h-max w-150 bg-white shadow-xl rounded-2xl">
      ...
      <div class="flex items-center justify-between py-5">
        ...
        <h2 v-if="(props.mode === 'view')" class="mt-2 relative left-12 text-4xl font-semibold">{{
          props.hashtagData.title }}<span v-if="(props.mode === 'edit')" class="text-[20px]"> ( Edit ) </span></h2>

        <button v-if="(props.mode === 'view')" class="absolute cursor-pointer right-134"
          @click="$emit('edit', props.hashtagObject)">
        ...
        <button @click="$emit('close')"
          class="relative right-8 text-2xl text-gray-400 cursor-pointer hover:text-gray-600">
      ...
      <form @submit.prevent="handleSave" :class="((props.mode === 'view') ? 'cursor-not-allowed ' : '') + 'p-8'">
        ...
        <div class="grid grid-cols-1">
          ...
          <HashtagInputs v-model:inputData="form.title" inputLabel="Hashtag" inputPlaceholder="Hashtag"
            :icon="personIcon" mustFill hasAsterisk validationMessage="Hashtag is required" :mode="props.mode" />
