...
const errorMessage = computed(() => {
  if (!fieldTouched.value && props.mode !== "edit") return null;
...
<template>
  <div :class="(isReadOnly ? 'cursor-not-allowed ' : '') + 'col-span-1 p-2'">
    <label
      :class="
        (isReadOnly ? 'opacity-70 ' : '') + 'block text-sm font-semibold mb-1'
      "
    >
      {{ props.inputLabel }}
    ...
    <div
      ...
      :class="
        (isReadOnly
          ? 'bg-gray-50 border-gray-200 border p-2.5 cursor-not-allowed '
          : 'bg-white border-gray-200 border rounded-lg p-2.5 ') +
    ...
    >
      ...
      <img
        :src="arrowDownIcon"
        class="w-3 h-3 relative top-1 right-3 opacity-100"
      />
    </div>
    ...
    <div
      ...
      :class="
        (hasError
          ? 'border-red-500 '
          : 'border-gray-200 focus-within:border-primary border') +
        (isReadOnly
          ? 'bg-gray-50 border-gray-200 border p-2.5 cursor-not-allowed '
          : 'bg-white border rounded-lg p-2.5 ') +
        'w-full flex items-center rounded-lg py-3'
      "
    >
    ...
    <div
      v-if="errorMessage && mode === 'edit'"
      class="text-red-500 text-xs font-normal mt-1 ml-1"
    >
      {{ errorMessage }}
    </div>
  </div>
