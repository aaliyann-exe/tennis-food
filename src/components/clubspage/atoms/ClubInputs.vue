...
<template>
  <div :class="(isReadOnly ? 'cursor-not-allowed ' : '') + 'col-span-1'">
    ...
    <div
      ...
      class="cursor-not-allowed flex bg-gray-50 w-full p-2.5 border border-gray-200 rounded-lg focus-within:border focus-within:border-primary"
    >
      <img
        :src="props.icon"
        class="cursor-not-allowed w-5 h-5 ml-1 mr-4 inline-block opacity-30"
      />

      <select
        disabled
        class="cursor-not-allowed inline text-primary font-light placeholder:text-black placeholder:opacity-10 outline-none w-full bg-transparent appearance-none"
      >
      ...
      <img
        :src="arrowDownIcon"
        class="w-3 h-3 relative top-2 right-2 opacity-60"
      />
    </div>
