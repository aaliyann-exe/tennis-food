    ...
    >
      <table class="w-full text-left border-collapse table-fixed">
          ...
          >
            <th class="px-6 py-4 w-[35%]">
              <div class="flex items-center gap-2">
                ...
                <span> {{ $t("court.court") }} {{ $t("table.name") }} </span>
              </div>
            </th>

            <th class="px-6 py-4 w-[40%] text-center">
              {{ $t("court.environment") }}
            </th>

            <th class="px-6 py-4 text-left w-[12%]">STATUS</th>
        ...
        <tbody class="divide-y divide-gray-300">
          <tr
            v-for="(court, index) in props.courts"
            :key="index"
          ...
          >
            <td class="px-6 py-4">
              <div class="flex items-center gap-4">
                <div
                  v-if="court.pfp"
                  class="shrink-0 w-10 h-10 overflow-hidden rounded-full"
                >
                  <img
                    :src="court.pfp"
                    alt="Club logo"
            ...
            <td class="px-6 py-4 text-right">
              <div class="flex items-center justify-end gap-3 opacity-50">
                ...
                <button
                  class="cursor-pointer"
                  @click="handleToggleStatus(court)"
                  :title="
                    court.status ? $t('table.activate') : $t('table.deactivate')
                  "
                >
                  <img
                    :src="court.status ? deactivateIcon : activateIcon"
