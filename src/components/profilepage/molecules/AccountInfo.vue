...
<template>
  <div
    class="h-full pb-5 border border-gray-300 rounded-3xl flexbox bg-secondary"
  >
    <div
      class="bg-linear-to-r from-[#fd6226] to-[#d9d9d9] p-8 rounded-t-3xl block w-full"
    ></div>
    ...
    <form @submit.prevent="handleSave" class="p-8">
      <div class="flex items-center justify-between px-8 py-5">
        ...
        <button
          v-if="mode === 'view'"
          class="p-2 transition rounded-full cursor-pointer hover:bg-gray-100"
      ...
      <div>
        <div class="grid grid-cols-2">
          <ProfileInputs
            ...
            :inputLabel="$t('table.fullName')"
            :inputPlaceholder="'Voer ' + $t('table.fullName') + ' in'"
            :icon="buildingIcon"
            mustFill
            hasAsterisk
            :validationMessage="
              $t('table.fullName') + ' ' + $t('modalField.error')
            "
            :mode="mode"
          />
          ...
          <ProfileInputs
            v-model:inputData="form.website"
            type="website"
            inputLabel="Website"
          ...
          <ProfileInputs
            ...
            :inputPlaceholder="$t('modalField.zipCode')"
            :icon="locationIcon"
            :mode="mode"
            hasAsterisk
          />
        ...
        <div class="grid grid-cols-2">
          <ProfileInputs
            v-model:inputData="form.repName"
            :inputLabel="
              'School ' +
              $t('table.representative').toLowerCase() +
              ' ' +
              $t('table.name').toLowerCase()
            "
            :inputPlaceholder="$t('modalField.rName')"
            :icon="personIcon"
            :mode="mode"
          />

          <ProfileInputs
            v-model:inputData="form.repEmail"
            type="email"
            :inputLabel="
              'School ' +
              $t('table.representative').toLowerCase() +
              ' ' +
              $t('table.email').toLowerCase()
            "
            :inputPlaceholder="$t('modalField.rEmail')"
            :icon="emailIcon"
            :mode="mode"
          />

          <ProfileInputs
            v-model:inputData="form.repPhone"
            type="phone"
            :inputLabel="
              'School ' +
              $t('table.representative').toLowerCase() +
              ' ' +
              $t('table.phone').toLowerCase()
            "
            :inputPlaceholder="$t('modalField.rPhone')"
            :icon="phoneIcon"
            :mode="mode"
          />
        </div>
